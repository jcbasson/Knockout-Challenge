...
class Tournament {
  ...
  static create(numberOfTeams, teamsPerMatch, options) {
    ...
    if (isNaN(numberOfTeams)) {
      throw new TournamentError(TOURNAMENT_ERROR_TYPE.INVALID, 'Oops, you forgot to give me the number of teams in the tournament.js');
    }

    const numberOfRounds = this.getNumberOfRounds(teamsPerMatch, numberOfTeams);

    if (numberOfRounds == null || numberOfRounds <= 0) {
      throw new TournamentError(TOURNAMENT_ERROR_TYPE.INVALID, 'Wait... you can\'t make a knockout tournament.js with that number of teams');
    }
  ...
  getMatch(round, match) {
    ...
    if (!this.matches[round]) {
      throw new TournamentError(TOURNAMENT_ERROR_TYPE.NOT_FOUND, 'That round does not exist in this tournament.js');
    }
